...
CREATE TABLE CLIENTE (
    ...
    CEP       CHAR(10),
    CPF       CHAR(20),
    FOREIGN KEY FK_CIDADE (CODCID) references CIDADE (CODCID) 
);
show tables;
...
select produto.descricao, qtdade from itempedido join produto on itempedido.CODPROD = produto.CODPROD order by itempedido.qtdade desc limit 1;

-- 11. Qual pedido teve a menor venda em quantidade? 
select produto.descricao, qtdade from itempedido join produto on itempedido.CODPROD = produto.CODPROD order by itempedido.qtdade asc limit 1;

SELECT * FROM vendas.produto;
-- 12. Qual o produto mais barato? 
select * from produto order by valor_un asc limit 1;

-- 13. Qual o produto mais caro? 
select * from produto order by valor_un desc limit 1;

-- 14. Quantos vendedores existem no setor de ferramentas? 
SELECT count(*) from vendedor join setor on vendedor.CODSETOR = setor.CODSETOR where vendedor.CODSETOR = '4';

-- 15. Quantos pedidos o cliente Estevan Pereira Cardoso fez? 
SELECT count(*) from pedido join cliente on pedido.CODCLI = cliente.CODCLI where cliente.NOME = 'Estevan Pereira Cardoso';